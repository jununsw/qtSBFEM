...
%   sdEdge{i}        - lines forming subdomain i, 
...
if ~iscell(sdConn) %ltx \texttt{sdConn} is a matrix (triangular elements) 
    ...
    meshEdge = sort(vertcat(meshEdge{:}),2);
    ...
    if size(sdConn{1},1) > 1 
        ...
        for ii = 1:nsd %ltx loop over polygons
            ...
            idx = find( meshEdge{ii}(:,1) > meshEdge{ii}(:,2));
            sdEdge{ii}(idx) = - sdEdge{ii}(idx); %ltx edge will be reversed
            i1 = i2 + 1; %ltx update the counter for the next element
        end
...
for ii = 1:nsd %ltx loop over subdomains/elements
    sdEdge{ii} = sign(sdEdge{ii}(:)).*ic(abs(sdEdge{ii})); %ltx update edge numbers
end
...
a = abs(cell2mat(sdEdge)); %ltx edges of all subdomains/elements
